...
from models.imagenet.resnet import BasicBlock
...
def duplicate_model_with_quant(model, bits, overflow_rate=0.0, counter=10, type='linear'):
    ...
    if isinstance(model, (nn.Conv2d, nn.Linear, nn.BatchNorm1d, nn.BatchNorm2d, nn.AvgPool2d, nn.GroupNorm)):
        l = nn.Sequential()
        if type == 'linear':
            quant_layer = LinearQuant('{}_quant'.format('conv'), bits=bits, overflow_rate=overflow_rate, counter=counter)
        elif type == 'log':
            quant_layer = NormalQuant('{}_quant'.format('conv'), bits=bits, quant_func=log_minmax_quantize)
        elif type == 'minmax':
            quant_layer = NormalQuant('{}_quant'.format('conv'), bits=bits, quant_func=min_max_quantize)
        else:
            quant_layer = NormalQuant('{}_quant'.format('conv'), bits=bits, quant_func=tanh_quantize)
        l.add_module(name='op',module=list(model.modules())[0])
        l.add_module(name='{}_{}_quant'.format('op',type),module=quant_layer)
        return l
    if isinstance(model, nn.Sequential) or isinstance(model,BasicBlock):
        l = OrderedDict()
        for k, v in model._modules.items():
            if isinstance(v, (nn.Conv2d, nn.Linear, nn.BatchNorm1d, nn.BatchNorm2d, nn.AvgPool2d, nn.GroupNorm)):
                l[k] = v
                if type == 'linear':
